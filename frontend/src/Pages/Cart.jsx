...
const Cart = () => {
    ...
    const {products,currency,cartItems,RemoveFromCart,getCartCount,updateCartItem,navigate,getCartAmount,axios,user,setCartItem}=useAppContext(); 
        ...
        try {
            ...
            if(paymentOption==="COD")
            {
               const {data}=await axios.post('api/order/cod',{
                userId:user._id,
                items:cartArray.map(item=>({products:item._id,quantity:item.quantity})),address:selectedAddress._id
               }) 
               if(data.success)
               {
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                <h1 className="text-3xl font-medium mb-6">
                    Shopping Cart <span className="text-sm text-green-600">{getCartCount()}</span>
                </h1>
...
<p className="text-gray-500">
  {selectedAddress
    ...
    : "No address found"}
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                               {addresses.map((address,index)=>(
                                ...
                                <p onClick={() =>navigate("/add-address")} className="text-green-600 text-center cursor-pointer p-2 hover:bg-indigo-500/10">
                                    Add address
                                </p>
                            </div>
                        )}
                ...
                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-green-600 text-white font-medium hover:bg-green-500 transition">
